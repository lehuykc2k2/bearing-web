...
import { ShieldCheck, Truck, BadgeDollarSign, Wrench, ArrowRight, Phone } from 'lucide-react'
...
export default async function HomePage() {
  ...
  const features = [
    { icon: ShieldCheck,     title: t('f1Title'), sub: t('f1Sub'), color: '#0BADE8' },
    { icon: Truck,           title: t('f2Title'), sub: t('f2Sub'), color: '#E5197E' },
    { icon: BadgeDollarSign, title: t('f3Title'), sub: t('f3Sub'), color: '#0BADE8' },
    { icon: Wrench,          title: t('f4Title'), sub: t('f4Sub'), color: '#E5197E' },
  ]

  const stats = [
    { num: t('stat1Num'), label: t('stat1Label'), sub: t('stat1Sub'), color: '#0BADE8' },
    { num: t('stat2Num'), label: t('stat2Label'), sub: t('stat2Sub'), color: '#E5197E' },
    { num: t('stat3Num'), label: t('stat3Label'), sub: t('stat3Sub'), color: '#0BADE8' },
  ]
  ...
  return (
    <>
      ...
      <section className="hero-shell relative overflow-hidden text-white">
        ...
        <div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#fbfdfe] to-transparent" />
        ...
        <div className="soft-enter relative max-w-6xl mx-auto px-5 py-14 md:py-24">
          ...
          <div className="inline-flex items-center gap-2 border text-[10px] md:text-xs font-bold px-3 py-1.5 rounded-full mb-4 uppercase tracking-wider backdrop-blur"
            style={{ borderColor: 'rgba(11,173,232,0.42)', color: '#8de4ff', background: 'rgba(6,26,48,0.58)' }}>
          ...
          <p className="text-sm md:text-xl leading-relaxed mb-3 md:max-w-xl" style={{ color: '#d6edf6' }}>
            {settings.banner_sub || settings.slogan}
          </p>
          <p className="text-[11px] md:text-sm font-semibold mb-7 tracking-widest uppercase" style={{ color: '#8de4ff' }}>
            {t('tagline')}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <Link href="/products"
              className="focus-ring interactive-lift flex items-center justify-center gap-2 font-bold px-6 py-3.5 rounded-lg text-sm text-white shadow-lg shadow-cyan-950/20"
              style={{ background: 'linear-gradient(135deg,#0BADE8 0%,#087fb5 100%)' }}>
              {t('viewProducts')} <ArrowRight size={15}/>
            </Link>
            {settings.phone && (
              <a href={`tel:${settings.phone.replace(/\s/g,'')}`}
                className="focus-ring interactive-lift flex items-center justify-center gap-2 font-bold px-6 py-3.5 rounded-lg text-sm text-white shadow-lg shadow-pink-950/20"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {features.map(({ icon: Icon, title, sub, color }) => (
              <div key={title} className="feature-tile interactive-lift rounded-lg p-3 md:p-5 flex items-center gap-3 md:flex-col md:items-center md:text-center">
                ...
                <div>
                  <div className="font-bold text-xs md:text-sm leading-snug" style={{ color: '#0A2340' }}>{title}</div>
        ...
        <section className="section-white py-9 md:py-14">
          <div className="max-w-6xl mx-auto px-3 md:px-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[10px] md:text-xs font-bold uppercase tracking-widest mb-0.5" style={{ color: '#0BADE8' }}>
                  {t('featuredLabel')}
                </p>
                <h2 className="text-xl md:text-3xl font-extrabold" style={{ color: '#0A2340' }}>
                  {t('featuredTitle')}
                </h2>
              </div>
              <Link href="/products"
      ...
      <section className="section-warm py-10 md:py-14 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-7">
            <p className="text-[10px] md:text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#0BADE8' }}>
              {t('whyLabel')}
            </p>
            <h2 className="text-xl md:text-3xl font-extrabold" style={{ color: '#0A2340' }}>
              {t('whyTitle')}
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 gap-3">
            {stats.map(({ num, label, sub, color }) => (
              <div key={label} className="stat-tile interactive-lift rounded-lg p-4 md:p-7 text-center">
                ...
                <div className="font-bold text-xs md:text-base leading-snug mb-0.5" style={{ color: '#0A2340' }}>{label}</div>
